<div class="notification" id="notification-icon" style="cursor: pointer; position: relative;">
    ...
    {{-- PERUBAHAN DI SINI: Menggunakan count($jadwalTerbaru) --}}
    @if(isset($jadwalTerbaru) && count($jadwalTerbaru) > 0)
        <span class="notif-badge">{{ count($jadwalTerbaru) }}</span>
    @endif
    ...
    <div class="notification-dropdown" id="notification-dropdown">
        ...
        <div class="notification-body" id="notificationBody">
            @forelse($jadwalTerbaru ?? [] as $notif)
                <div class="notification-item" data-notif-id="{{ $notif->id ?? ($notif->matakuliah . '_' . $notif->jam) }}">
                    <div class="notif-icon-wrapper">
                        <i class="fa-solid fa-calendar-day"></i>
                    </div>
                    <div class="notif-content">
                        {{-- Jika ke depannya ada error "Attempt to read property on array", ubah $notif->... menjadi $notif['...'] --}}
                        <p>Jadwal <strong>{{ Str::limit($notif->matakuliah, 25) }}</strong> telah diperbarui menjadi jam <strong>{{ $notif->jam }}</strong>.</p>
                        @if(isset($notif->updated_at))
                        <small>{{ \Carbon\Carbon::parse($notif->updated_at)->diffForHumans() }}</small>
                        @endif
                    </div>
                </div>
            @empty

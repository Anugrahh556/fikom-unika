...
<body>
    <aside class="sidebar">
        <div class="sidebar-top">
            <img src="<?php echo e(asset('Logo_UNIKA.png')); ?>" alt="Logo" />
            <div class="logo-text"><h2>UNIVERSITAS KATOLIK SANTO THOMAS</h2><p>SISTEM PENJADWALAN MATAKULIAH</p></div>
        </div>
        <ul class="menu">
            <li class="active" onclick="window.location.href = '<?php echo e(url('/dashboard')); ?>'"><i class="fa-solid fa-table-cells-large"></i> Dashboard</li>
            <li onclick="window.location.href = '<?php echo e(url('/jadwal')); ?>'"><i class="fa-regular fa-calendar"></i> Jadwal</li>
            <li onclick="window.location.href = '<?php echo e(url('/dosen')); ?>'"><i class="fa-solid fa-users"></i> Dosen</li>
            <li onclick="window.location.href = '<?php echo e(url('/ruangan')); ?>'"><i class="fa-solid fa-door-open"></i> Ruangan</li>
        </ul>
    </aside>
    ...
    <main class="main-content">
        <header class="topbar">
            ...
            <div class="topbar-right">
                <?php echo $__env->make('partials.notification_bell', ['jadwalTerbaru' => $jadwalTerbaru ?? []], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <?php echo $__env->make('partials.profile_dropdown', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
        </header>

        <?php if($userRole == 'admin' || $userRole == 'dosen'): ?>
            <div class="welcome-card">
                <div class="welcome-text"><h2>Selamat Datang, <?php echo e(strtok(Auth::user()->name, ' ')); ?>!</h2><p>Anda login sebagai <strong><?php echo e(strtoupper(Auth::user()->role)); ?></strong>. Gunakan menu navigasi untuk mengelola sistem.</p></div>
            </div>

            <section class="cards">
                <div class="card"><div class="card-icon maroon"><i class="fa-solid fa-users"></i></div><div class="card-text"><h5>Total Dosen Aktif</h5><h2><?php echo e($totalDosen ?? 0); ?></h2></div></div>
                <div class="card"><div class="card-icon orange"><i class="fa-solid fa-book-open-reader"></i></div><div class="card-text"><h5>Total Mata Kuliah</h5><h2><?php echo e($totalMatkul ?? 0); ?></h2></div></div>
                <div class="card"><div class="card-icon green"><i class="fa-solid fa-door-open"></i></div><div class="card-text"><h5>Total Ruangan Fisik</h5><h2><?php echo e($totalRuangan ?? 0); ?></h2></div></div>
            </section>
                ...
                <div class="right-content">
                    ...
                    <div class="content-box" style="animation-delay: 0.6s; opacity: 0; animation: fadeInUp 0.6s ease-out forwards;">
                        <div class="ruangan-header">
                            <a href="<?php echo e(url('/ruangan')); ?>">
                                <h2>Ruangan Terpakai Saat Ini</h2>
                                <i class="fa-solid fa-arrow-right-long"></i>
                            </a>
                        </div>
                        <div id="room-status-container">
                            <?php $__empty_1 = true; $__currentLoopData = $ruanganTerpakai ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ruangan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <div class="room-item"><div class="room-left"><div class="room-icon blue-bg"><i class="fa-solid fa-chalkboard-user"></i></div><div class="room-text"><h4>Ruang <?php echo e($ruangan->ruang); ?></h4><p><?php echo e(Str::limit($ruangan->matakuliah, 20)); ?></p></div></div><span class="room-status blue-status">Dipakai</span></div>
        ...
        <?php elseif($userRole == 'mahasiswa'): ?>
            <div class="welcome-card"><div class="welcome-text"><h2>Selamat Datang, <?php echo e(strtok(Auth::user()->name, ' ')); ?>!</h2><?php if(isset($mahasiswa)): ?><p>Anda login sebagai mahasiswa <strong><?php echo e($mahasiswa->jurusan ?? 'N/A'); ?> - Kelas <?php echo e($mahasiswa->id_kelas ?? $mahasiswa->kelas ?? 'N/A'); ?> (Semester <?php echo e($semesterRomawi ?? '-'); ?>)</strong>.</p><?php endif; ?></div></div>

            <?php if(isset($jadwalBerikutnya) && $jadwalBerikutnya): ?>
                <div class="reminder-card" data-countdown="<?php echo e($detikMenujuKelas); ?>"><i class="reminder-icon fa-solid fa-bell fa-shake"></i><div class="reminder-content"><p class="reminder-title">JADWAL KULIAH ANDA BERIKUTNYA</p><h3 class="reminder-matkul"><?php echo e($jadwalBerikutnya->matakuliah); ?></h3><div class="reminder-details"><span><i class="fa-solid fa-chalkboard-user"></i> <?php echo e($jadwalBerikutnya->dosen->nama ?? 'N/A'); ?></span><span><i class="fa-solid fa-location-dot"></i> Ruang <?php echo e($jadwalBerikutnya->ruang); ?></span><span><i class="fa-regular fa-clock"></i> <?php echo e($jadwalBerikutnya->jam); ?></span></div></div><div class="reminder-countdown"><p>DIMULAI DALAM</p><div id="countdown-timer">00:00:00</div></div></div>
            <?php else: ?>
                <div class="reminder-card empty"><i class="reminder-icon fa-solid fa-circle-check"></i><div class="reminder-content"><h3 class="reminder-matkul">Tidak Ada Jadwal Kuliah Lagi Untuk Hari Ini</h3><p style="opacity: 0.8; font-size: 0.9rem;">Semua kelas Anda untuk hari <?php echo e($hariIni); ?> telah selesai. Selamat beristirahat!</p></div></div>
            <?php endif; ?>

            <section class="cards">
                <div class="card"><div class="card-icon maroon"><i class="fa-solid fa-hourglass-half"></i></div><div class="card-text"><h5>Total SKS Hari Ini</h5><h2><?php echo e($totalSksHariIni ?? 0); ?></h2></div></div>
                <div class="card"><div class="card-icon orange"><i class="fa-solid fa-book-open-reader"></i></div><div class="card-text"><h5>Total Matakuliah</h5><h2><?php echo e($totalMatkulHariIni ?? 0); ?></h2></div></div>
                <div class="card"><div class="card-icon green"><i class="fa-solid fa-check-double"></i></div><div class="card-text"><h5>Matakuliah Selesai</h5><h2><?php echo e($matkulSelesai ?? 0); ?></h2></div></div>
            </section>

            <div class="dashboard-grid">
                <div class="left-content">
                    <div class="content-box">
                        <div class="table-header"><h2>Jadwal Kuliah Anda Hari Ini (<?php echo e($hariIni); ?>)</h2></div>
                        <div class="table-responsive">
                            <table>
                                ...
                                <tbody>
                                    <?php $__empty_1 = true; $__currentLoopData = $jadwalHariIni; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jadwal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <tr>
                                            <td><?php echo e($jadwal->jam); ?></td><td><strong><?php echo e($jadwal->matakuliah); ?></strong><br><small class="kelas-badge"><?php echo e($jadwal->sks); ?> SKS</small></td><td><?php echo e($jadwal->dosen->nama ?? 'N/A'); ?></td><td><?php echo e($jadwal->ruang); ?></td>
                                            <td><span class="status <?php echo e($jadwal->status_class); ?>"><?php echo e($jadwal->status); ?></span></td>
                                        </tr>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        ...
                        <div class="pagination-container">
                            <?php if(isset($jadwalHariIni) && $jadwalHariIni instanceof \Illuminate\Pagination\LengthAwarePaginator && $jadwalHariIni->hasPages()): ?>
                                <div class="pagination-info">Showing <strong><?php echo e($jadwalHariIni->firstItem() ?? 0); ?></strong> to <strong><?php echo e($jadwalHariIni->lastItem() ?? 0); ?></strong> of <strong><?php echo e($jadwalHariIni->total() ?? 0); ?></strong> entries</div>
                                <div class="pagination-wrapper"><?php echo e($jadwalHariIni->links('pagination::bootstrap-4')); ?></div>
                            <?php else: ?>
                                <?php $totalItems = isset($jadwalHariIni) ? $jadwalHariIni->total() : 0; ?>
                                <div class="pagination-info">Showing <strong><?php echo e($totalItems > 0 ? 1 : 0); ?></strong> to <strong><?php echo e($totalItems); ?></strong> of <strong><?php echo e($totalItems); ?></strong> entries</div>
    ...
    <script>
        ...
        window.jadwalHariIniData = <?php echo json_encode(
            isset($jadwalHariIni)
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function initStatistikChart() {
         ...
         const chartData = <?php echo json_encode($grafikData ?? [0, 0, 0, 0, 0]); ?>;
const chartLabels = <?php echo json_encode($grafikLabels ?? ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat']); ?>;
            new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: chartLabels,
...
</html><?php /**PATH C:\laragon\www\fikom-unika\resources\views/dashboard.blade.php ENDPATH**/ ?>